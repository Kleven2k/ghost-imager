...
module uart_interface #(
    ...
    // Physical UART pins
    input  wire logic rx_pin,
    output wire logic tx_pin,
    ...
    output logic        rx_msg_done,      // pulses on CRC byte (every packet, incl. zero-len)
    output logic        rx_crc_ok,        // CRC result, valid when rx_msg_done

    // -- TX (FPGA -> PC) -----------------------------------------------
    // Contract: caller must hold tx_msg_type, tx_msg_len, and tx_payload_byte
    // (byte 0) valid before asserting tx_send. tx_payload_req then pulses to
    // advance to byte 1, 2, ..., N-1. The req for byte N (the last) is not
    // emitted — the FSM goes straight to CRC.
    input  wire logic [7:0]  tx_msg_type,      // message type to send
    input  wire logic [15:0] tx_msg_len,       // payload length (0 = no payload)
    input  wire logic [7:0]  tx_payload_byte,  // next payload byte from caller
    output logic             tx_payload_req,   // pulse: advance to next byte
    input  wire logic        tx_send,          // pulse to start sending a packet
            ...
            if (raw_rx_valid) begin
                case (rx_state)
                    ...
                    RX_GET_PAYLOAD: begin
                        ...
                        if (rx_byte_count == rx_msg_len - 1)
                            rx_state <= RX_GET_CRC;
                    end

                    // rx_msg_done pulses with rx_crc_ok on the CRC byte —
                    // fires for every packet, including zero-length payloads,
                    // and signals "packet fully validated, here's the result."
                    RX_GET_CRC: begin
