...
# Must match the parameter overrides in uart_interface_tb_wrapper.sv
CLK_HZ    = 1_000_000
BAUD      = 115_200
BIT_TICKS = CLK_HZ // BAUD  # = 8 cycles per bit

# -- Helpers --------------------------------

async def reset(dut):
    """Assert reset (active-low) for 4 cycles, deassert, then wait one more rising edge."""
    dut.rst_n.value           = 0
...
async def test_tx_packet(dut):
    ...
    # Contract: byte 0 must be on tx_payload_byte before tx_send.
    # tx_payload_req then advances 0->1, 1->2, ..., (N-2)->(N-1).
    async def drive_remaining_payload():
        for b in payload[1:]:
    ...
    dut.tx_payload_byte.value = payload[0]  # byte 0 pre-loaded

    cocotb.start_soon(drive_remaining_payload())
